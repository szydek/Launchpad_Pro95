...
class SCScaleComponent(ControlSurfaceComponent):
	...
	def __init__(self, control_surface = None, enabled = False, mode = "diatonic", *a, **k):
		...
		super(SCScaleComponent, self).__init__(*a, **k)
	...
	def update(self):
		if self.is_enabled() and self._matrix!=None:
			...
			super(SCScaleComponent, self).update()
			self._update_OSD()
			#for index, button in enumerate(self._matrix):
			for button, (col, row) in self._matrix.iterbuttons():
				...
				if row==0:
					if col == 0:
						...
						button.set_light("Scale.RelativeRoot") # was DefaultButton.Disabled
					elif col == 2:
