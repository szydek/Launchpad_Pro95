			...
			if self._clip != None:
				...
				for x in range(8):
					...
					for y in range(7):
						if self._mode == STEPSEQ_MODE_NOTES:
							if self._notes_pitches[(x + 8 * self._page) * 7 + 6 - y] == 1:
								self._grid_back_buffer[x][y] = "StepSequencer3.Pitch.On"
							else:
								self._grid_back_buffer[x][y] = "StepSequencer3.Pitch.Off"

						elif self._mode == STEPSEQ_MODE_NOTES_OCTAVES:
							if(has_note):
								if self._notes_octaves[x + 8 * self._page] == 6 - y:
									self._grid_back_buffer[x][y] = "StepSequencer3.Octave.On"
								else:
									self._grid_back_buffer[x][y] = "StepSequencer3.Octave.Off"
							else:
								if self._notes_octaves[x + 8 * self._page] == 6 - y:
									self._grid_back_buffer[x][y] = "StepSequencer3.Octave.Dim"
								else:
									self._grid_back_buffer[x][y] = "StepSequencer3.Octave.Off"
							...
							if(has_note):
								if self._notes_velocities[x + 8 * self._page] >= 6 - y:
									self._grid_back_buffer[x][y] = "StepSequencer3.Velocity.On"
								else:
									self._grid_back_buffer[x][y] = "StepSequencer3.Velocity.Off"
							else:
								if self._notes_velocities[x + 8 * self._page] >= 6 - y:
									self._grid_back_buffer[x][y] = "StepSequencer3.Velocity.Dim"
								else:
									self._grid_back_buffer[x][y] = "StepSequencer3.Velocity.Off"
							...
							if has_note:
								if self._notes_lengths[x + 8 * self._page] >= 6 - y:
									self._grid_back_buffer[x][y] = "StepSequencer3.Length.On"
								else:
									self._grid_back_buffer[x][y] = "StepSequencer3.Length.Off"
							else:
								if self._notes_lengths[x + 8 * self._page] >= 6 - y:
									self._grid_back_buffer[x][y] = "StepSequencer3.Length.Dim"
								else:
									self._grid_back_buffer[x][y] = "StepSequencer3.Length.Off"
				# metronome
				if self._playhead != None:
					...
					if self._mode == STEPSEQ_MODE_NOTES_LENGTHS:
						if page == self._page:
							metronome_color = "StepSequencer3.NoteEditor.MetronomeInPage"
						else:
							metronome_color = "StepSequencer3.NoteEditor.MetronomeInOtherPage"
					else:
						if page == self._page:
							metronome_color = "StepSequencer3.NoteEditor.MetronomeInPage"
						else:
					...
					if self._mode == STEPSEQ_MODE_NOTES:
						for y in range(7):
							if self._notes_pitches[play_position * 7 + 6 - y] == 1:
								if page == self._page:
									self._grid_back_buffer[play_x_position][y] = "StepSequencer3.NoteEditor.PlayInPage"
								else:
									self._grid_back_buffer[play_x_position][y] = "StepSequencer3.NoteEditor.PlayInOtherPage"
	...
	def _update_random_button(self):
		if self.is_enabled():
			if (self._random_button != None):
				if self._clip != None:
					self._random_button.set_on_off_values("StepSequencer3.Random.On", "StepSequencer3.Random.Off")
	...
	def _update_mode_notes_pitches_button(self):
		if self.is_enabled():
			if (self._mode_notes_pitches_button != None):
				if self._clip != None:
					self._mode_notes_pitches_button.set_on_off_values("StepSequencer3.Pitch.On", "StepSequencer3.Pitch.Dim")
					if self._mode == STEPSEQ_MODE_NOTES:
	...
	def _update_mode_notes_octaves_button(self):
		if self.is_enabled():
			if (self._mode_notes_octaves_button != None):
				if self._clip != None:
					self._mode_notes_octaves_button.set_on_off_values("StepSequencer3.Octave.On", "StepSequencer3.Octave.Dim")
	...
	def _update_mode_notes_velocities_button(self):
		if self.is_enabled():
			if (self._mode_notes_velocities_button != None):
				if self._clip != None:
					self._mode_notes_velocities_button.set_on_off_values("StepSequencer3.Velocity.On", "StepSequencer3.Velocity.Dim")
	...
	def _update_mode_notes_lengths_button(self):
		if self.is_enabled():
			if (self._mode_notes_lengths_button != None):
				if self._clip != None:
					self._mode_notes_lengths_button.set_on_off_values("StepSequencer3.Length.On", "StepSequencer3.Length.Dim")
					if self._mode == STEPSEQ_MODE_NOTES_LENGTHS:
...
class StepSequencerComponent3(StepSequencerComponent):

	def __init__(self, control_surface = None, side_buttons = None, top_buttons = None):
		super(StepSequencerComponent3, self).__init__(control_surface)
