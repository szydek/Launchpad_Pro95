...
from .SCScaleComponent import SCScaleComponent
...
class SCNoteComponent(ControlSurfaceComponent):
	...
	def __init__(self, control_surface = None, feedback_channels = [0,1,2], non_feedback_channel = 15, get_pattern = None, *a, **k):
		...
		# self._stz_get_pattern = stz_get_pattern
		self._control_surface = control_surface
		self._layout_set = False
		self._non_feedback_channel = non_feedback_channel
		self._feedback_channels = feedback_channels
		super(SCNoteComponent, self).__init__(*a, **k)

	
	def update_matrix_mapping(self, playable = False): # this actualy defines the matrix layout
		...
		for index, button in enumerate(self.matrix):
			...
			if index > 17 and index < 24:
				...
				button.channel = 15
				button.set_playable(True)
				# note_channel[index] = 0
			'''if note_info.index != None and note_info.index<128:
				if note_info.root:
					button.color = "Note.Pads.OutOfScale" # was Root
					...
					button.color = "Note.Pads.OutOfScale" # Invalid
	...
	def set_matrix(self, matrix): # THIS SETS UP THE PADS!
